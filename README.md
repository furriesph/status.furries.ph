...
The Node collector is scheduled through GitHub Actions every 10 minutes and publishes `status.json` and `feed.xml` to the public `status-data` branch. This makes at most 144 scheduled Cloudflare-account collections per day; browser refreshes only read the published feed every 10 minutes and do not call Cloudflare account APIs. Hosted schedules and raw-file CDN caching can delay fresh data; the page's stale indicator is part of the operating model. Enable Actions with `GITHUB_TOKEN` contents-write permission, then verify a successful run and its committed data. Supabase project probes and Cloudflare account analytics require collector-only credentials. Copy `.env.monitor.example` to the ignored `.env.monitor.local`; `pnpm monitor` loads this and ignored `.env.cloudflare.local`. Follow the runbook for local Wrangler refresh and GitHub secret mappings. Public provider incident feeds need no credentials.

Deploy as a static Cloudflare Pages project using `pnpm build` and output directory `build/client`. Configure `VITE_STATUS_URL` as `https://raw.githubusercontent.com/OWNER/REPO/status-data/status.json` and `VITE_FEED_URL` as the corresponding `feed.xml` URL, replacing the repository placeholders. The public repository or a public data mirror must be browser-readable: never add a browser token. Observation updates do not require a site rebuild. Local development defaults to `/status.json`. Set up `status.furries.ph` in the hosting dashboard only after a preview passes verification.
