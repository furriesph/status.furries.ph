...
Replaced the incorrect inferred 10,000,000 monthly Workers inclusion with the verified account dashboard allowances: 100,000 Workers requests per day, 200,000 Observability events per day and 3,000 Workers build minutes per month. The request card now uses a real account-wide UTC-day analytics count and adds CPU time. The status page also renders the known Observability/build allowances without fabricating their current use: the existing credential receives HTTP 403 from the Workers Observability API, which requires Cloudflare's Workers Observability Write-labelled permission for telemetry queries, and no permitted endpoint exposes build-minute consumption. The repository variables are set, tests, typecheck, validation and build are pending final deployment verification.

### 2026-09-13 - Correct Cloudflare account and account telemetry

Replaced the collector account and zone configuration with the active production Cloudflare account. Live inventory now confirms 6 Worker scripts, 54 Pages projects, 1 KV namespace, 1 R2 bucket and 4 Durable Object namespaces. The request-limits component retains real account-wide quota data when optional zone analytics or product meters are unavailable. It now runs Cloudflare's bounded aggregate Observability query with the required token permission and displays its result as a quota progress bar when Cloudflare returns a verified count; build-minute use remains unavailable because the public collector API does not return it. The current suite has 73 passing tests; typecheck and production build passed. Local fixture validation is intentionally blocked by an old bundled sample that predates the newly added Sanity service; scheduled publication validates the current complete snapshot.
