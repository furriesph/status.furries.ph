...
- Show verified Cloudflare dashboard allowances for daily Workers requests, daily Observability events and monthly build minutes. Daily request and CPU time use come from Workers analytics; Observability events use Cloudflare's bounded account telemetry query when it returns a verified aggregate. A missing optional zone or product meter stays explicit without hiding the live account-wide data.
