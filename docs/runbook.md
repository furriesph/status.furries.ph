...
3. Create the Cloudflare Pages project for the static SPA. Build with `pnpm build`; serve `build/client`. Set `VITE_STATUS_URL=https://raw.githubusercontent.com/OWNER/REPO/status-data/status.json` and `VITE_FEED_URL=https://raw.githubusercontent.com/OWNER/REPO/status-data/feed.xml`, replacing the repository placeholders. Use the public repository or a public data mirror; never put an access token in the browser. Confirm SPA fallback routing.
...
| CLOUDFLARE_API_TOKEN | Secret environment variable | Authorized read-only analytics token |
...
Configure the zone for response-code coverage. Lower daily/monthly operating budgets are optional; when none are set and the live Worker usage model is Standard, the collector publishes actual UTC-day Worker requests, records that Standard has no Cloudflare daily hard request cap, and derives its included monthly allotment. Each collection also inventories Worker scripts, Pages projects, KV namespaces, R2 buckets, Durable Object namespaces, D1 databases and Queues through the account API. It reports the actual daily-limit rule for every active product: Pages Functions share Workers usage while static assets are free and unlimited; Paid KV, R2 and Paid Durable Objects use monthly included usage rather than a daily hard quota. Product billing usage remains unavailable unless the collector receives separate Billing Read access. Missing permissions, empty analytics, an unrecognized usage model or missing zone scope remain explicit Unknown monitoring gaps unless independent direct evidence proves impact. Analytics estimate observed usage and are not billing totals or a guarantee that every account limit is covered. Worker checks inspect a 15-minute window for runtime errors/resource-limit outcomes. Request checks warn at 80% of an explicitly configured operating threshold and on observed zone 429/server errors.

For the live plan-derived path, the Cloudflare collector token needs read access to the account Worker settings in addition to analytics and zone-response data. The collector reads the configured Worker `usage_model`; a live `standard` value supplies the published 10,000,000 included monthly request allotment. This is an included-usage threshold, not a hard outage limit or an invoice total. Billing Read remains optional and is only needed to inspect subscriptions or invoices.
...
Operational probes use count-only database HEAD requests over a 15-minute window. Email covers blast/provider-acceptance and planning outbox records; linked messaging covers Discord and Telegram planning receipts. Background jobs cover asset webhooks and the planning outbox; social publishing covers recorded provider publication, not federation inbox delivery. Failures and overdue work cause degradation; idle paths without recent successes remain Unknown. Payment confirmations may be manual, so they never certify external checkout availability. LAN requires a recent active-session heartbeat and projected operation pair. SMS checks fresh permitted devices, verified active SIMs and recorded delivery receipts without sending a message.
