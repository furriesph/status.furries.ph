...
| LAN | No active session/heartbeat and recent projected-operation pair | Verify during an actual connected LAN session; no artificial session is created |
...
| Publishing and jobs | Queues are readable with no recent successes, failures or overdue work | Current execution health remains Unknown without recent receipts; empty queues alone do not prove workers executed |
